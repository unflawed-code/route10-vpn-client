#!/bin/sh
# wg/mac-test.sh - Test suite for MAC address utilities
# Migrated from route10-wireguard-client/tests/wg-pbr-suite

SCRIPT_DIR="$(dirname "$(readlink -f "$0")")"
PROJECT_ROOT="$(dirname "$(dirname "$SCRIPT_DIR")")"
# In new structure, utils are in lib/common.sh
COMMON_LIB="${PROJECT_ROOT}/lib/common.sh"
TEMP_DIR="/tmp/r10-wg-mac-test-$$"
...
# --- Deduplication tests (Uses resolve_targets from wg.sh?) ---

# The dedupe test in legacy relied on 'simulate_dedupe' function defined inside the test.
# We can keep that self-contained logic here to verify the 'algorithm',
# OR we can try to load `wg.sh` resolve_targets.
# Let's keep the explicit algorithm check as it's safe.
